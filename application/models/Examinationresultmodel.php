			 ...
			 if(empty($row))
			 {
			  ...
			  { }
				        $id=$rows->subject;
						 //echo $id;
						// $id=$rows->subject;
					   $sQuery = "SELECT t.teacher_id,t.class_teacher,su.*,en.enroll_id,en.name FROM edu_subject AS su,edu_teachers AS t,edu_enrollment AS en WHERE t.teacher_id='$teacher_id' AND en.class_id='$cls_masid'";
					   $objRs=$this->db->query($sQuery);
					   $rows=$objRs->result();
					...
					$datas= array("status" => "Success","subject_id"=>$sub_id,"subject_name"=>$sub_name,"enr_id"=>$enrid,"stu_name"=>$stu_name1,"cls_id"=>$cls_id1);
