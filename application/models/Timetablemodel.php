      ...
      parent::__construct();
       ...
       //GET ALL TERMS
              ...
              function create_timetable($year_id,$term_id,$class_id,$subject_id,$teacher_id,$day_id,$period_id){
                  $check="SELECT * FROM edu_timetable WHERE class_id='$class_id'";
                ...
                function view_class_timetable(){
                  $query="SELECT tt.class_id AS timid,cm.class_sec_id,cm.class,cm.section,c.class_id,c.class_name,s.sec_name FROM edu_timetable AS tt  INNER JOIN edu_classmaster AS cm ON tt.class_id=cm.class_sec_id INNER JOIN edu_class AS c ON cm.class=c.class_id INNER JOIN edu_sections AS s ON cm.section=s.sec_id GROUP BY c.class_name";
