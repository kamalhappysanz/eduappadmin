...
Class Examinationmodel extends CI_Model
{
  ...
  function get_exam_details()
	 ...
	 {
		 $query="select ex.exam_detail_id,ex.subject_id,ex.exam_date,ex.classmaster_id,cm.	class_sec_id,ex.notes,s.subject_name,c.class_name,se.sec_name  FROM edu_exam_details AS ex,edu_classmaster AS cm,edu_subject AS s,edu_class AS c,edu_sections AS se WHERE  ex.subject_id=s.subject_id AND ex.classmaster_id=cm.	class_sec_id AND c.class_id =cm.class AND se.sec_id=cm.section";
    ...
    function exam_details($exam_year,$exam_name)
    {
	 ...
	 function add_exam_details($exam_year,$class_name,$subject_name,$exam_date,$notes)
	{
	  $check_exam_name="SELECT * FROM edu_exam_details WHERE exam_id='$exam_year' AND subject_id='$subject_name' AND classmaster_id='$class_name'";
	  $result=$this->db->query($check_exam_name);
      if($result->num_rows()==0)
	  {
		$query="INSERT INTO edu_exam_details(exam_id,subject_id,exam_date,classmaster_id,notes,status,created_at) VALUES ('$exam_year','$subject_name','$exam_date','$class_name','$notes','A',NOW())";
		$resultset=$this->db->query($query);
        $data= array("status" => "success");
        return $data;
		}else{
            $data= array("status"=>"Exam Already Added");
            return $data;
          }
	...
	function update_exam_details($id,$exam_year,$class_name,$subject_name,$formatted_date,$notes)
	{
	    $query="UPDATE edu_exam_details SET exam_id='$exam_year',subject_id='$subject_name',exam_date='$formatted_date',classmaster_id='$class_name',notes='$notes',updated_at='NOW()' WHERE exam_detail_id='$id' ";
		$res=$this->db->query($query);
